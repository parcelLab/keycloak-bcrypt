...
public class BCryptPasswordHashProvider implements PasswordHashProvider {
    ...
    public String encode(final String rawPassword, final int iterations) {
        ...
        return BCrypt.with(BCrypt.Version.VERSION_2B).hashToString(cost, rawPasswordAsSHA256String.toCharArray());
    }
    ...
    public boolean verify(final String rawPassword, final PasswordCredentialModel credential) {
        ...
        String rawPasswordAsSHA256String = Hashing.sha256().hashString(rawPassword, StandardCharsets.UTF_8).toString();
        final BCrypt.Result verifier = BCrypt.verifyer(BCrypt.Version.VERSION_2B).verify(rawPasswordAsSHA256String.toCharArray(), hash.toCharArray());
